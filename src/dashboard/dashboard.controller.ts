...
export class DashboardController {
  ...
  @ApiOperation({
    summary: '이번 달 절약 금액',
    description:
      '이번 달 절약 금액(원화 표시)과 전년 동월 대비 절약 금액/증감 안내를 반환합니다.',
  })
  @ApiResponse({
    status: 200,
    description: '조회 성공',
    type: SavingsMetricResponseDto,
  })
  @ApiResponse({
    status: 401,
    description: '인증 실패',
  ...
  @ApiOperation({
    ...
    description:
      '최근(이번 달 우선, 없으면 최근 6개월) 지출 합계가 가장 높은 쇼핑몰 1곳을 반환합니다.',
  })
  @ApiResponse({
    status: 200,
    description: '조회 성공',
    type: TopMerchantMetricResponseDto,
  })
  @ApiResponse({
    status: 401,
    description: '인증 실패',
  ...
  @ApiOperation({
    ...
    description:
      '클라이언트 호환을 위한 alias 엔드포인트입니다. 응답 스키마는 top-merchant와 동일합니다.',
  })
  @ApiResponse({
    status: 200,
    description: '조회 성공',
  ...
  @ApiOperation({
    ...
    description:
      '연결된 결제수단 중 이번 달 사용 금액 기준으로 Top1을 반환합니다. (데이터가 없으면 최근 6개월 기준으로 보정)',
  })
  @ApiResponse({
    status: 200,
    description: '조회 성공',
    type: TopPaymentMethodMetricResponseDto,
  })
  @ApiResponse({
    status: 401,
    description: '인증 실패',
  ...
  @ApiOperation({
    summary: 'AI 추천 혜택(요약)',
    description:
      '최근 6개월 소비 요약 + 등록 결제수단 + 혜택(크롤링) 데이터를 종합해 AI 추천 결과/사유를 반환합니다.',
  })
  @ApiResponse({
    status: 200,
    description: '조회 성공',
    type: AiBenefitSummaryMetricResponseDto,
  })
  @ApiResponse({
    status: 401,
    description: '인증 실패',
  ...
  @ApiOperation({
    ...
    description:
      '최근 6개월의 월간 지출/혜택(절약) 합계를 JSON으로 반환합니다. 절약 지표 계산은 서비스 함수로 분리되어 있습니다.',
  })
  @ApiResponse({
    status: 200,
    description: '조회 성공',
    type: MonthlySavingsTrendResponseDto,
  })
  @ApiResponse({
    status: 401,
    description: '인증 실패',
  ...
  @ApiOperation({
    ...
    description:
      '혜택 크롤링 데이터 + 소비 패턴을 종합해 AI가 추천하는 결제수단 Top3를 반환합니다.',
  })
  @ApiResponse({
    status: 200,
    description: '조회 성공',
    type: RecommendedPaymentMethodsTop3ResponseDto,
  })
  @ApiResponse({
    status: 401,
    description: '인증 실패',
  ...
  @ApiOperation({
    ...
    description:
      '사이트명/결제일/결제수단/할인·적립 금액을 포함한 최근 결제 내역을 페이지 단위로 반환합니다.',
  })
  @ApiResponse({
    status: 200,
    description: '조회 성공',
    type: RecentSiteTransactionsResponseDto,
  })
  @ApiResponse({
    status: 401,
    description: '인증 실패',
  ...
  async getRecentTransactionsBySite(
    @Req() req: any,
    @Query() query: RecentSiteTransactionsQueryDto,
  ) {
    const userUuid = req.user.uuid;
