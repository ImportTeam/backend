...
export class AnalyticsService {
  ...
  private parseDateOrThrow(value?: string, fieldName?: string) {
    ...
    if (Number.isNaN(d.getTime())) {
      throw new BadRequestException(`${fieldName || '날짜'} 형식이 올바르지 않습니다.`);
    }
    return d;
  }

  async getCategorySpendingLastSixMonths(userUuid: string) {
    const now = new Date();
  ...
  async getMonthlySpendingTrendLastSixMonths(userUuid: string) {
    const now = new Date();
  ...
  async getTransactions(userUuid: string, query: AnalyticsTransactionsQueryDto) {
    const now = new Date();
    const defaultRange = this.getLastSixMonthsRange(now);

    const from = this.parseDateOrThrow(query.from, 'from') ?? defaultRange.start;
    ...
    if (query.paymentMethodIds && query.paymentMethodIds.length > 0) {
      where.payment_method_seq = { in: query.paymentMethodIds.map((id) => BigInt(id)) };
    }
